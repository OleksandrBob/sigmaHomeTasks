...
namespace StorageUPD
{
    ...
    {
        Product DeleteSingleBadProduct();
        void AddProduct(Product product);
        (int, int, int) MaxLimits { get; set; }
        (int, int, int) CurrentCount { get; set; }

    }


    public abstract class StorageDecorator : IControllable
    {
        protected IControllable _controllable = null;
        protected string _filePath = null;
        public (int, int, int) MaxLimits { get; set; }
        public (int, int, int) CurrentCount { get; set; }

        public StorageDecorator(IControllable controllable, string filePath)
        {
        ...
        public abstract Product DeleteSingleBadProduct();
    }


    public class TaskDecorator : StorageDecorator
    {
        public TaskDecorator(IControllable controllable, string filePath) : base(controllable, filePath) { }

        public override void AddProduct(Product product)
        {
            if (product is MeatProduct && _controllable.MaxLimits.Item2 < _controllable.CurrentCount.Item2 + 1)
            {
                Console.WriteLine("Too many 'Meat' objects in storage");
                return;
            }
            else if (product is DairyProduct && _controllable.MaxLimits.Item3 < _controllable.CurrentCount.Item3 + 1)
            {
                Console.WriteLine("Too many 'Dairy' objects in storage");
                return;
            }
            else if (product is Product && _controllable.MaxLimits.Item1 < _controllable.CurrentCount.Item1 + 1) 
            {
                Console.WriteLine("Too many 'Dairy' objects in storage");
                return;
            }

            _controllable.AddProduct(product);
        }

        public override Product DeleteSingleBadProduct()
        {
            var deletedProduct = _controllable.DeleteSingleBadProduct();
            if (deletedProduct != null)
            {
                try
                {
                    ...
                    {
                        sw.WriteLine($"deleted bad product - {deletedProduct}");
                    }
                }
                catch (Exception) 
                {
                    Console.WriteLine("Impossible to open this file");
                }
            }
    ...
    public class Storage: IEnumerable<Product>, IControllable
    {
        private List<Product> products;
        private (int, int, int) koefs;
        ...
        public event ProblemWithProduct ProblemWithProduct;
        ...
        {//This method reinitialise last 'List<Product>' element
            ...
            Storage thisObject = sender as Storage;
            if (thisObject == null)
            {
                throw new Exception("Invalid use of the function. First parameter should be a 'Storage'");
            }
        ...
        public void FindCurrentCount() 
        {
            int countOfDairy = 0;
            int countOfClassic = 0;
            int countOfMeat = 0;
            ...
            CurrentCount = (countOfClassic, countOfMeat, countOfDairy);
        }
        ...
        public Product DeleteSingleBadProduct()// Task 13 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        {
        ...
        public void AddRange(Product[] productArray)
        {
            foreach (Product product in productArray)
            {
                ...
                {
                    products.Add(null);
                    ProblemWithProduct?.Invoke(this, "Impossible to add unexisting product to the list");                    
            ...
            try
            {
                using (StreamReader sr = new StreamReader(filePath))
                {
                    ...
                    try
                    {
                        ...
                        this.koefs.Item1 = Convert.ToInt32(koefs[0]);
                        this.koefs.Item2 = Convert.ToInt32(koefs[1]);
                        this.koefs.Item3 = Convert.ToInt32(koefs[2]);
                    }
                    catch (Exception)
                    {
                    ...
                    for (int i = 1; i < initialisationParameters.Length; i++)
                    {
                        ...
                        {//Event invoke !!!
                            ProblemWithProduct?.Invoke(this, $"Impossible to parse product data({productInitialisationParameters[1].Substring(0, productInitialisationParameters[1].Length - 1)})");
                        }
                    }
