...
namespace StoreProgram
{
    ...
    class AdminManager : IAdminManagerForModerator, IAdminManagerForAdministrator
    {
        ...
        {
            return (administrators.Find((adm) => adm.Login == login) != null) && (moderators.Find((mod) => mod.Login != login) == null);
        }

        public bool RegisterNewModerator(string name, string login, string password, string VerifyAdminRegistration)
        {
            if (VerifyAdminRegistration != "swalddnm")
            {
                return false;//Access denied - to register a moderator we should know speccial password
            }
        ...
        public bool RegisterNewAdministrator(string name, string login, string password, string VerifyAdminRegistration)
        {
            if (VerifyAdminRegistration != "swalddnm")
            {
                return false;//Access denied - to register a moderator we should know speccial password
